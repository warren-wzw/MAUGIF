...
DATA=f'/home/BlueDisk/Dataset/FusionDataset/RGBT/FMB/test/'
...
def main():
    ...
    with torch.no_grad():
        for file in files:
            ...
            vis_  = convert_tensor(image1).to('cuda:0')
            ir = convert_tensor(image2).to('cuda:0').unsqueeze(0)
            if vis_.size(0)==1:
                vis=vis_.unsqueeze(0)
            else:
                Y_vis,Cb_vis,Cr_vis= RGB2YCrCb(vis_.unsqueeze(0))
                vis=Y_vis
